  ...
  const fetchMovies = async (query = '') => {
    ...
    try {
      if (query.trim()) {
        ...
        // Update trending collection in Appwrite (non-blocking)
        updateSearchCount(query, data.results[0]).catch((err) =>
          console.error('Appwrite update failed:', err)
        )
      } else {
        // HOMEPAGE DISCOVER
        const baseEndpoint = `${API_BASE_URL}/discover/movie?sort_by=popularity.desc&include_adult=false&language=en-US`
        const [page1, page2] = await Promise.all([
          fetch(`${baseEndpoint}&page=1`, API_OPTIONS),
          fetch(`${baseEndpoint}&page=2`, API_OPTIONS),
        ])

        if (!page1.ok || !page2.ok) throw new Error('Failed to fetch movies')

        const data1 = await page1.json()
        const data2 = await page2.json()

        setMovieList([...(data1.results || []), ...(data2.results || [])])
      }
    } catch (error) {
    ...
    <main>
      ...
      <div className="wrapper">
        ...
        <section className="all-movies">
          <h2>{debouncedSearchTerm ? `Results for "${debouncedSearchTerm}"` : 'All Movies'}</h2>
          {isLoading ? (
            <Spinner />
          ) : errorMessage ? (
